    ...
    <div class="container-fluid card px-4 pt-2">
        ...
        <div class="tab-content" id="pills-tabContent">
            ...
            <div class="tab-pane fade show active" id="faqList" role="tabpanel"
                ...
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Manage Exam Faq's</h3>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <?php $count = 1; ?>
                                @foreach ($examFaqList as $item)
                                    <tr>
                                        <td><?php echo $count++; ?></td>
                                        <td>{{ $item->examFaqId }}</td>
                                        <td>{{ $item->examName }}</td>
                                        ...
                                        <td class="d-flex justify-content-center align-items-center">
                                            <a href="#editFaq" data-faq-id="{{ $item->examFaqId }}" role="tab"
                                            ...
                                            <div class="d-flex ml-3" style="margin-top: -2px;">
                                                @if ($item->faqStatus == 0)
                                                    <form action="/updateExamFaqStatus/{{ $item->examFaqId }}" method="post">
            ...
            <div class="tab-pane fade" id="addFaq" role="tabpanel" aria-labelledby="pills-addFaq-tab">
                <form autocomplete="off" action="{{ route('admin.addExamFaq') }}" method="POST"
                    ...
                    <div class="form-group ">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <select class="form-control " name="faqExamId" id="faqExamId"
                                required="required">
                                <option class="faqExamId" value="0" selected disabled>Select Exam</option>
                                @foreach ($examList as $item)
                                    <option class="faqExamId" value="{{ $item->examId }}">
                                        {{ $item->examName }}
                                    </option>
                                @endforeach
                            </select>
            ...
            <div class="tab-pane fade " id="editFaq" role="tabpanel" aria-labelledby="pills-editFaq-tab">
                <form autocomplete="off" id="updateExamFaqForm" method="POST" class="x_content new_x_content"
                    ...
                    <div class="form-group ">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <select class="form-control " name="faqExamId" id="edit_faqExamId"
                                required="required">
                                <option class="faqExamId" value="0" selected disabled>Select Exam</option>
                                @foreach ($examList as $item)
                                    <option class="faqExamId" value="{{ $item->examId }}">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.editFaqBtn').on('click', function() {
                ...
                var url = "{{ route('admin.fetchExamFaqContent', ':examFaqId') }}";
                url = url.replace(':examFaqId', faq_id);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#edit_faqExamId').val(data.faqExamId);
                        $('#edit_faqQuestion').val(data.faqQuestion);  
                        $('#edit_faqAnswer').summernote('code', data.faqAnswer);

                        $('#updateExamFaqForm').attr('action',
                            "{{ route('admin.updateExamFaq', '') }}/" + faq_id);
                    },
