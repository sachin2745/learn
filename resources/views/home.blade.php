@php
    $data = DB::table('pages')->where('pageType', 1)->first();
@endphp
...
@section('main')
        ...
        <div class="carousel-inner">
            ...
            @foreach ($banner as $item)
                <div class="carousel-item active">
                    <img src="{{ $item->bannerImgSmall }}" class="imgbig w-100 " alt="{{ $item->bannerImgAlt }}">
                    <img src="{{ $item->bannerImgLarge }}" class="imgsmall w-100 " alt="{{ $item->bannerImgAlt }}">
                </div>
            @endforeach
            ...
            <div class="platform-innerCont">
                <div class="position-relative platform-innCont">
                    <img alt="hero-student" fetchPriority="high" loading="eager" width="320" height="225"
                        ...
                        src="{{ asset('assets/happy_student_m.png') }}" />
                    ...
                    <div class="platform-cont">
                        <div class="position-absolute d-flex" style="width: 20px;height:100%;right:-10px;z-index:-1;top:0;">
                    ...
                    <div class="d-flex postion-relative flex-row rounded overflow-hidden innerCard">
                        <div class="d-flex flex-column innerCardLeft">
                            ...
                            <div class="d-flex flex-row align-items-start flex-wrap overflow-y-auto buttonContainer">
                                @if (count($exam->batches) > 0)
                                    @foreach ($exam->batches as $batch)
                                        <a href="{{ route('CourseDetail', $batch->batchSlug) }}"
                                            class="sec03-button cursor-pointer"> {{ $batch->batchName }}</a>
                                    @endforeach
                                @endif
                            </div>
                            <a href="{{ route('Courses', $exam->examSlug) }}"
                                class="d-flex flex-row align-items-center buttonCont group mt-auto">
                        ...
                        <div class="d-flex align-items-center innerCardRight">
                            <div class="innerCardRightSec">
                                <img src="{{ asset($exam->examIcon) }}" alt="" lazy="loading" width="40"
                                    height="40" class="">
                            </div>
                        </div>
                    </div>
    ...
    <div class="sec05">
        <div>
            ...
            <div class="sec05_btnCont">
                <div class="sec05_Cont px-3">
                    @foreach ($result as $item)
                        <div class="sec05_btn d-md-block d-none" data-img-src="{{ $item->examResultBanner }}">
                            <span>{{ $item->examName }}</span>
                        </div>
                        <div class="sec05_btn d-md-none d-block" data-img-src="{{ $item->examResultBannerM }}">
                            <span>{{ $item->examName }}</span>
                        </div>
                    @endforeach
                </div>
                    ...
                    <div id="customDynamicCarousel" class="carousel slide" data-bs-ride="carousel">
                        <div>
                            <div class="carousel-inner custom-carousel-inner ">
                                @foreach ($result as $item)
                                    <div class="carousel-item active">
                                        <img src="{{ $item->examResultBanner }}" class="d-md-block d-none w-100 h-auto"
                                            alt="{{ $item->examBannerAlt }}">
                                        <img src="{{ $item->examResultBannerM }}" class="d-md-none d-block w-100 h-auto"
                                            alt="{{ $item->examBannerAlt }}">
                                    </div>
                                @endforeach
                            </div>
        ...
        <div>
            ...
            <div class="sec08_testCont">
                <div class="position-relative">
                    <img src="{{ $review[0]->reviewStudentImg }}" alt="{{ $review[0]->reviewStudentName }} Image"
                        loading="lazy" width="0" decoding="async" height="0" class="sec08_testImg">
                            ...
                            <div>
                                <div class="sec08_testCard">
                                    ...
                                    <div class="d-flex gap-3 align-items-center">
                                        <div
                                            style="padding:4px;border-color:#d9dce1;border-width: 2px;border-radius: 9999px;">
                                            <img src="{{ $item->reviewStudentImg }}"
                                                alt="{{ $item->reviewStudentName }} Image" loading="lazy" width="0"
                                ...
                                height:36px;width:36px;background-repeat: no-repeat;background-size: cover; background-position: center;overflow:hidden;border-radius: 9999px;">
                                        ...
                                        <div>
                                            <div
                                                style="font-weight:500;color:#1b2124;font-size: 1rem;line-height: 1.5rem;">
                                                {{ $item->reviewStudentName }}
                                            </div>
                                            <div class="d-flex gap-2">
                                                <div class=""
                                                    style="border-color: rgb(0 0 0);font-weight: 600;color:#5a4bda;font-size: 0.75rem;line-height: 1rem;">
                                                    {{ $item->reviewRank }}</div>
                                                <span style="color: #E4E4E7;margin-top:-4px;">|</span>
                                                <div
                                                    style="color:#5a4bda;font-size: 0.75rem;line-height: 1rem;font-weight: 600;">
                                                    {{ $item->reviewExam }}</div>
                                            </div>
                                        </div>
                                    </div>
    ...
    <style>
        ...
        .sec07-carousel-slider {
            display: flex;
            animation: sec07-slide 30s infinite linear;
        }
        ...
        @keyframes sec07-slide {
            ...
            100% {
                transform: translateX(calc(-360px * {{ count($platform) }}));
            }
        }
    ...
    <div class="sec07">
        <div class="sec07_head text-center">
            Join India Most Loved Educational Platform Today
        </div>
        <div class="sec07_subhead text-center">
            Explore our 72 YouTube Channels and subscribe to get access to quality education for free.
        </div>


        @if (count($platform) < 3)
            <!-- Show a single image if less than 3 images -->
            <div class="sec07-single-image mt-3 ">
                @foreach ($platform as $item)
                    <a href="{{ $item->platformLink }}">
                        <img src="{{ $item->platformImg }}" alt="{{ $item->platformAlt }}">
                    </a>
                @endforeach
            </div>
        @else
            <div class="sec07-carousel-container">
                <div class="sec07-carousel-slider">
                    ...
                    <!-- Cloning the slides for infinite loop -->
                    @foreach ($platform as $item)
                        <div class="sec07-carousel-slide">
                            <a href="{{ $item->platformLink }}">
                                <img src="{{ $item->platformImg }}" alt="{{ $item->platformAlt }}" />
